...
const destinations = [
  { id: 1, name: 'شیرینی میخک', image: mikhak, rating: 4.5 },
    ...
    <div className="flex flex-col py-12">
      <div className="w-full px-4 sm:px-6 lg:px-1">
        ...
        <div className="relative md:hidden">
          <button
            onClick={scrollPrev}
            className="absolute left-0 top-1/2 transform -translate-y-1/2 z-10 bg-white/70 hover:bg-white/90 rounded-full w-8 h-8 flex items-center justify-center shadow-md ml-2"
          >
            <ChevronLeftIcon className="h-5 w-5" />
          </button>
          <button
            onClick={scrollNext}
            className="absolute right-0 top-1/2 transform -translate-y-1/2 z-10 bg-white/70 hover:bg-white/90 rounded-full w-8 h-8 flex items-center justify-center shadow-md mr-2"
          >
